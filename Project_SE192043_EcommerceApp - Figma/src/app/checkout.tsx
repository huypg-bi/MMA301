...
const PAYMENT_METHODS = [
  { icon: 'voucher', label: 'Credit / Debit Card' },
  { icon: 'tag', label: 'Apple Pay' },
  { icon: 'door_to_door_delivery', label: 'Cash on Delivery' },
];
      ...
      >
          ...
          {items.map((item) => (
            <View key={item.product.id} style={styles.orderItem}>
              <View style={styles.orderItemLeft}>
                ...
                <Text style={styles.orderItemName} numberOfLines={1}>{item.product.title}</Text>
              </View>
        ...
        <View style={styles.section}>
          ...
          {PAYMENT_METHODS.map((method, index) => (
            <Pressable
              key={method.label}
              style={[styles.paymentOption, index === selectedPayment && styles.paymentOptionSelected]}
            ...
            >
              <SvgIcon
                ...
                width={22}
                height={22}
                color={index === selectedPayment ? COLORS.secondary : COLORS.body}
              />
              <Text style={[styles.paymentLabel, index === selectedPayment && styles.paymentLabelSelected]}>
                {method.label}
              </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.successDetails}>
              ...
              <Text style={styles.successDetailText}>{form.address}, {form.city}</Text>
            ...
            <View style={styles.successSummary}>
              ...
              <Text style={styles.successSummaryValue}>{items.reduce((s, i) => s + i.quantity, 0)}</Text>
            </View>
            <View style={styles.successSummary}>
...
function FormField({ label, value, onChangeText, placeholder, error, multiline, ...rest }: FieldProps) {
  return (
    <View style={fieldStyles.container}>
      <Text style={fieldStyles.label}>{label}</Text>
      <TextInput
        style={[fieldStyles.input, error && fieldStyles.inputError, multiline && fieldStyles.multiline]}
        value={value}
